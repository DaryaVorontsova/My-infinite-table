...
export const RecordConstructor: React.FC<{ onSuccess?: () => void }> = ({ onSuccess }) => {
  ...
  return (
    <form className="border rounded p-3 mb-4" onSubmit={handleSubmit} style={{maxWidth: 550}}>
      ...
      <div className="mb-2">
        <button type="button" className="btn btn-secondary me-2" onClick={addField} disabled={submitting}>
          + Добавить поле
        </button>
      </div>
      {formError && <div className="alert alert-danger py-1">{formError}</div>}
      <button type="submit" className="btn btn-primary" disabled={submitting || fields.length < 5}>
